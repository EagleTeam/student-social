...
class VietCalendar {
  ...
  int getLeapMonthOffset(int a11, double timeZone) {
    ...
    int arc =
        INT(getSunLongitude(getNewMoonDay(k + i, timeZone), timeZone) / 30);
    do {
      last = arc;
      i++;
  ...
  List<int> lichAm(int dd, int mm, int yy) {
    double TZ = 7.0;
    int start = jdFromDate(dd, mm, yy);
    int step = 15;
    int count = -1;
    while (count++ < 240) {
      int jd = start + step * count;
      List<int> s = jdToDate(jd);
      List<int> l = convertSolar2Lunar(s[0], s[1], s[2], TZ);
      List<int> s2 = convertLunar2Solar(l[0], l[1], l[2], l[3], TZ);
